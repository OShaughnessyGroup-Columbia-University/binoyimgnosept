...
function fname_gro = matlab_to_gro_fin3_march(filename)
...
% loading the file   
    ...
    myp_tail = 130; %myp2 tail length (in nm)
    myp_tail = myp_tail/10;
    myp_num = 16;
    ...
    if exist('rmyp') ==1
        tot_num = (lvl1*3*myol) + (2*myol) + (lvl1*2*myol)+ len +...
			length(ifor)+ ((5*myp_num)*length(rmyp(1,:))) + crosslen + length(rmyp(1,:));
    else
    ...
    for i = 1:len
        mol(i,:) = 'CA';
	if exist('highlightFlag') == 1
            if highlightFlag(i) == true
		mol(i,:) = 'CS';
	    end
	end
    end
    ...
    if exist('rmyp') == 1 && isempty(rmyp) == 0 %to account for runs without myp2 present
        ...
        myp_points((5*myp_num*myplen),3) = 0;
        myp_points_len = length(myp_points(:,1));
        
        sphere_p = 5 * myp_num; 
  
        
        %inserting 8 dimers at correct locations, all points generated by myp2_shape_march.m
        % added to the variable myp_points. for details of creation see myp2_shape_march file 

        for i = 1:sphere_p:(myp_points_len)
            myp_points(i:(i+sphere_p-1),:) = myp2_shape_march(rmyptrans(ceil(i/sphere_p),1),rmyptrans(ceil(i/sphere_p),2),...
                   rmyptrans(ceil(i/sphere_p),3),myp_num, myp_tail); 
        end
        ...
        %identity of each bead to corresponding dimer (first 65 beads to first sphere and so on)
        m_id = ((len+1):myp_points_len);
        m_id = m_id.';

       for i = 1:65:myp_points_len
           m_id(i:(i+64))= ceil(i/65);       
       end
      ...
      myp_name(2:5:myp_points_len,2) = 'M';
      myp_name(3:5:myp_points_len,2) = 'M';
      myp_name(4:5:myp_points_len,2) = 'E';
      myp_name(5:5:myp_points_len,2) = 'H';    
   ...
   %gromacs column printing
    ...
    %myp2 sphere centers ton control color and radius, named CB (same as
    %myo2 anchor bead)
    mypa = myoh_ind(end);
    myp_a = 1:myplen;
    myp_a = myp_a.';
    mypa_ind = (mypa+1: mypa + myplen);
    mypa_ind = mypa_ind.';
	
	if exist('rmyp') == 1 && isempty(rmyp) == 0 %to account for runs without myp2 present
		mypa_main = [mypa_ind rmyptrans];
		for i = 1:length(rmyptrans(:,1))
			fprintf(fileID, fmt,myp_a(i),'MYOAN','   CP',mypa_main(i,:));
		end
    end
    ...
    if exist('rmyp') == 1 && isempty(rmyp) == 0
        for i = 1:myplen
           fprintf(fileIDp, fmtp,mypa_ind(i),'MYP',1,'CP','CP',1,0.000000,1.0000,0);      
        end 
    ...
    %arranging bonds for myp2 dumbell structures   
        ...
        mypbond(1:5:myp_points_len,1)= m_ind(1:5:myp_points_len);
        mypbond(1:5:myp_points_len,2)= m_ind(2:5:myp_points_len);
        mypbond(2:5:myp_points_len,1)= m_ind(2:5:myp_points_len);
        mypbond(2:5:myp_points_len,2)= m_ind(3:5:myp_points_len);
